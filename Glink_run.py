...
class UCIe_2p5D:
    ...
    def Hardware_Training_Non(self, **kargs):
        ...
        if data_training_en != 0:
            for idx in range(len(self.modes)):
                ...
                if eye_scan == '2d' :
                    self.phy.rg_vref_range_start(self.tx_die, self.tx_group, self.tx_group_n, setv=vref_start, slice=self.tx_slice)
                    self.phy.rg_vref_range_start(self.rx_die, self.rx_group, self.rx_group_n, setv=vref_start, slice=self.rx_slice)
                    self.phy.rg_vref_range_num(self.tx_die, self.tx_group, self.tx_group_n, setv='0x0', slice=self.tx_slice)
                    self.phy.rg_vref_range_num(self.rx_die, self.rx_group, self.rx_group_n, setv='0x0', slice=self.rx_slice)
                else:
                    vref_start = '0x00 ~ 0x3F'

                print(f'Init Slice : TX Slice=User Define / RX Slice=User Define (Test Die : Only Force Die0 and Die1)')
                self.phy.reg_user_set(die_arr=self.die_arr, group_arr=self.group_arr, tx_slice=self.tx_slice, rx_slice=self.rx_slice, reg_arr=hw_non_1, mode=init_mode)
    ...
    def PCS_BIST_Check_NON(self, **kargs):
        ...
        for idx in range(len(self.modes)):
            ...
            self.phy.reg_user_set(die_arr=self.die_arr, group_arr=self.group_arr, tx_slice=self.tx_slice, rx_slice=self.rx_slice, reg_arr=PCS_BIST_Check_NON, mode=init_mode)

            # inject error
            print('Function Check : RX_PCS_ERR_INJECT : Enable', flush=True)
            self.phy.MONITOR_CLR(self.tx_die, self.tx_group, slice=self.tx_slice, setv='0x1')
            self.phy.MONITOR_CLR(self.rx_die, self.rx_group, slice=self.rx_slice, setv='0x1')
            self.phy.RX_PCS_BIST_COMPARE(self.tx_die, self.tx_group, slice=self.tx_slice, setv='0x1')
            self.phy.RX_PCS_BIST_COMPARE(self.rx_die, self.rx_group, slice=self.rx_slice, setv='0x1')
            self.phy.TX_PCS_BIST_RUN(self.tx_die, self.tx_group, slice=self.tx_slice, setv='0x1')
            self.phy.TX_PCS_BIST_RUN(self.rx_die, self.rx_group, slice=self.rx_slice, setv='0x1')
            self.phy.RX_PCS_ERR_INJECT(self.tx_die, self.tx_group, slice=self.tx_slice, setv='0x1', r_bk=1)
            self.phy.RX_PCS_ERR_INJECT(self.rx_die, self.rx_group, slice=self.rx_slice, setv='0x1', r_bk=1)
            error_count_inject = self.PCS_BIST_Check_NON_result(mode=init_mode, skip_result=1)
            if avdd_sense_en == 1:
                time.sleep(0.1)
            ...
            if len(self.tx_slice) * 2 == error_count_inject:
                self.inject_chk = ''
                print(f'\034Function Check : RX_PCS_ERR_INJECT Result : PASS', flush=True)
            elif error_count_inject == 0:
                self.inject_chk = '_Error (Valid_Failed)'
                print(f'\033Function Check : RX_PCS_ERR_INJECT Result : Error_(Valid_Failed)', flush=True)
            else:
                self.inject_chk = '_Error (Inject_Failed)'
                print(f'\033Function Check : RX_PCS_ERR_INJECT Result : Failed', flush=True)
            ...
            if data_replay == 1:
                ...
                self.Replay_CHK = ''
                self.phy.RX_PCS_RPLY_en(self.tx_die, self.tx_group, slice=self.tx_slice, setv='0x1')
                self.phy.TX_PCS_RPLY_en(self.rx_die, self.rx_group, slice=self.rx_slice, setv='0x1')
                self.phy.MONITOR_CLR(self.tx_die, self.tx_group, slice=self.tx_slice, setv='0x1')
                self.phy.MONITOR_CLR(self.rx_die, self.rx_group, slice=self.rx_slice, setv='0x1')
                self.phy.RX_PCS_BIST_COMPARE(self.tx_die, self.tx_group, slice=self.tx_slice, setv='0x1')
                self.phy.RX_PCS_BIST_COMPARE(self.rx_die, self.rx_group, slice=self.rx_slice, setv='0x1')
                ...
                self.phy.RX_PCS_ERR_INJECT(self.tx_die, self.tx_group, slice=self.tx_slice, setv='0x1', r_bk=0)
                self.phy.RX_PCS_ERR_INJECT(self.rx_die, self.rx_group, slice=self.rx_slice, setv='0x1', r_bk=0)
                self.PCS_BIST_Check_NON_result(mode=init_mode, skip_result=0)
                self.phy.RX_PCS_BIST_COMPARE(self.tx_die, self.tx_group, slice=self.tx_slice, setv='0x0')
                self.phy.RX_PCS_BIST_COMPARE(self.rx_die, self.rx_group, slice=self.rx_slice, setv='0x0')
                self.phy.TX_PCS_BIST_RUN(self.tx_die, self.tx_group, slice=self.tx_slice_sw, setv='0x0')
                self.phy.TX_PCS_BIST_RUN(self.rx_die, self.rx_group, slice=self.rx_slice_sw, setv='0x0')
            ...
            for L in range(int(chk_loop)):
                ...
                if str(chk_time).find('pi') != -1:
                    ...
                    print(f'PCS BIST Time : Check Loop {L + 1}/{chk_loop} , Time {chk_time}s', flush=True)
                    time.sleep(int(chk_time))

                error_count_inject = self.PCS_BIST_Check_NON_result(mode=init_mode, skip_result=0)
    ...
    def PCS_BIST_Check_NON_result(self, **kargs):
        ...
        for idx in range(len(self.modes)):
            ...
            tx_pcs_val = self.phy.BIST_ERR_COUNT(self.tx_die, self.tx_group, slice= self.tx_slice)
            rx_pcs_val = self.phy.BIST_ERR_COUNT(self.rx_die, self.rx_group, slice= self.rx_slice)
            if skip_result == 0:
                for P in range(len(tx_pcs_val)):
                    if tx_pcs_val[P] != 0:
                        print(f'\033Die{self.tx_die}{self.tx_group_n} Slice{self.tx_slice[P]}, Error Count={tx_pcs_val[P]}, Faiied', flush=True)
                    else:
                        print(f'\034Die{self.tx_die}{self.tx_group_n} Slice{self.tx_slice[P]}, Error Count={tx_pcs_val[P]}', flush=True)
                    if rx_pcs_val[P] != 0:
                        print(f'\033Die{self.rx_die}{self.rx_group_n} Slice{self.rx_slice[P]}, Error Count={rx_pcs_val[P]}, Failed', flush=True)
                    else:
                        print(f'\034Die{self.rx_die}{self.rx_group_n} Slice{self.rx_slice[P]}, Error Count={rx_pcs_val[P]}', flush=True)
            else:
                for P in range(len(tx_pcs_val)):
                    if tx_pcs_val[P] != 0:
                        print(f'\033Die{self.tx_die}{self.tx_group_n} Slice{self.tx_slice[P]}, Error Count={tx_pcs_val[P]}', flush=True)
                    else:
                        print(f'\034Die{self.tx_die}{self.tx_group_n} Slice{self.tx_slice[P]}, Error Count={tx_pcs_val[P]}', flush=True)
                    if rx_pcs_val[P] != 0:
                        print(f'\033Die{self.rx_die}{self.rx_group_n} Slice{self.rx_slice[P]}, Error Count={rx_pcs_val[P]}', flush=True)
                    else:
                        print(f'\034Die{self.rx_die}{self.rx_group_n} Slice{self.rx_slice[P]}, Error Count={rx_pcs_val[P]}', flush=True)
    ...
    def VCO(self,**kargs):
        ...
        REX_arr = []
        tx_speed_arr = []
        self.visa.PG_81160A_2CH(CMU1_Amp=0.5, CMU1_S=100000000, CMU2_Amp=0.5, CMU2_S=100000000)
        for k in range(CMU_loop):
            CMU_Freq = CMU_S + (CMU_G * k)
            self.visa.PG_81160A_2CH(CMU_S=CMU_Freq)
            time.sleep(1)
            REXT = self.visa.M5_34411A_Voltage(visa='USB0::0x2A8D::0x1301::MY57223676::0::INSTR') # Die0 V1
            REX_arr += [REXT]
            print(f'RCLK Frequency={CMU_Freq}Hz , REXT_Did0_V1={REXT}V', flush=True)

            sel_div = self.phy.cfg_pre_div_sel(self.tx_die, self.tx_group, self.tx_slice,  doset=0, slice_num=-1)  # vco Die0 V2 --> self.EHOST[0][2]
    ...
    def check_speed(self, **kargs):
        ...
        reg_chk = ['0x0/0x28/0x0/0x0/0x28/0x0', '0x0/0x1E/0x0/0x0/0x1E/0x0', '0x1/0x28/0x1/0x1/0x28/0x1']
        tx_check1 = self.phy.rg_pmaa_MODE_8_target(self.tx_die, self.tx_group, slice= self.tx_slice, doset=0, r_bk=1)
        tx_check2 = self.phy.cfg_sel_div_target(self.tx_die, self.tx_group, slice= self.tx_slice, doset=0, r_bk=1)
        tx_check3 = self.phy.cfg_vco_div_mode_target(self.tx_die, self.tx_group, slice= self.tx_slice, doset=0, r_bk=1)
        rx_check1 = self.phy.rg_pmaa_MODE_8_target(self.rx_die, self.rx_group, slice= self.rx_slice, doset=0, r_bk=1)
        rx_check2 = self.phy.cfg_sel_div_target(self.rx_die, self.rx_group, slice= self.rx_slice, doset=0, r_bk=1)
        rx_check3 = self.phy.cfg_vco_div_mode_target(self.rx_die, self.rx_group, slice= self.rx_slice, doset=0, r_bk=1)
        reg_value = f'{tx_check1}/{tx_check2}/{tx_check3}/{rx_check1}/{rx_check2}/{rx_check3}'
    ...
    def Read_pi_value(self, **kargs):
        ...
        self.phy.cfg_rpt_cck_phase(self.tx_die, self.tx_group,  slice= self.tx_slice_sw, setv='0x1')
        self.phy.cck_rpt_code_i_reg(self.tx_die, self.tx_group, slice= self.tx_slice_sw)
        self.phy.cfg_rpt_cck_phase(self.tx_die, self.tx_group,  slice= self.tx_slice_sw, setv='0x0')
        self.phy.cfg_rpt_cck_phase(self.tx_die, self.tx_group,  slice= self.tx_slice_sw, setv='0x1')

        self.phy.cfg_rpt_cck_phase(self.rx_die, self.rx_group, slice= self.rx_slice_sw, setv='0x1')
        self.phy.cck_rpt_code_i_reg(self.rx_die, self.rx_group, slice= self.rx_slice_sw)
        self.phy.cfg_rpt_cck_phase(self.rx_die, self.rx_group,  slice= self.rx_slice_sw, setv='0x0')
        self.phy.cfg_rpt_cck_phase(self.rx_die, self.rx_group,  slice= self.rx_slice_sw, setv='0x1')
    def Read_pi_status_die0_V(self, times):
        # Check pi point
        for g in range(times):
            self.phy.cfg_rpt_cck_phase(self.tx_die, self.tx_group,  slice= self.tx_slice, setv='0x1')
            self.phy.cfg_rpt_cck_phase(self.rx_die, self.rx_group, slice= self.rx_slice, setv='0x1')
            ...
            self.phy.cfg_rpt_cck_phase(self.tx_die, self.tx_group,  slice= self.tx_slice, setv='0x0')
            self.phy.cfg_rpt_cck_phase(self.rx_die, self.rx_group, slice= self.rx_slice, setv='0x0')
            self.phy.cfg_rpt_cck_phase(self.tx_die, self.tx_group,  slice= self.tx_slice, setv='0x1')
            self.phy.cfg_rpt_cck_phase(self.rx_die, self.rx_group, slice= self.rx_slice, setv='0x1')
    ...
    def read_center_vref(self, **kargs):
        ...
        getattr(self, mode)()  # run Mx_mode()
        tx_center_vref = self.phy.cfg_vref_sel_rxgp(self.tx_die, self.tx_group, doset=0, slice= self.tx_slice)
        rx_center_vref = self.phy.cfg_vref_sel_rxgp(self.rx_die, self.rx_group, doset=0, slice= self.rx_slice)
    ...
    def avdd_sense(self, **kargs):
        ...
        if meas_voltage < 0.5:
            ...
            print(f'Chip Internal Voltage Value={meas_voltage}V / Sense Voltage={sense_voltage}', flush=True)
            ...
            self.phy.TPSM831D31_VoltageSet(0x4, 'CHA', sense_voltage)  # D0_AVDD_V1_D1_V1
            # self.phy_0.TPSM831D31_VoltageSet(0x4, 'CHB', volt)  # D0_AVDD12_V1_D1_V1
            self.phy.TPSM831D31_VoltageSet(0x8, 'CHA', sense_voltage)  # D1_AVDD_V2_D2_V1
    ...
    def avdd_voltage_sense(self, **kargs):
        ...
        print(f'Slice Voltage Sense : Die{self.tx_die} (PLL)AVDD Slice0 Voltage={pll_avdd[0]}V , (PLL)AVSS Slice0 Voltage={pll_avss[0]}V', flush=True)
        print(f'Slice Voltage Sense : Die{self.rx_die} (PLL)AVDD Slice0 Voltage={pll_avdd[1]}V , (PLL)AVSS Slice0 Voltage={pll_avss[1]}V', flush=True)
        ...
        if pll_volt > 1 or pll_volt < 0.5:
            ...
            if mode == 'M2SN_mode' or mode == 'M2DIE2_0_FLB_mode' or mode == 'M2DIE0_2_FLB_mode':
                self.phy.D0_AVDD_V1_D2_V1_set(pll_volt)
                print(f'D0_AVDD_V1_D2_V1(PMIC) Sense Voltage(PLL) = {pll_volt}', flush=True)
            elif mode == 'M2EW_mode' or mode == 'M2DIE2_1_FLB_mode' or mode == 'M2DIE1_2_FLB_mode':
                self.phy.D1_AVDD_V2_D2_V2_set(pll_volt)
                print(f'D1_AVDD_V2_D2_V2(PMIC) Sense Voltage(PLL) = {pll_volt}', flush=True)
            else:
                pass
    ...
    def even_message(self, **kargs):
        ...
        app.geometry('+50+50')
        rdio_1 = tk.Radiobutton(app, variable=radioValue, value=0, text='Check M4_D0V_D1V_mode Die0 Pi Status 100 times')
        rdio_2 = tk.Radiobutton(app, variable=radioValue, value=1, text='Check M4_D0V_D1V_mode Die1 Pi Status 100 times')
        rdio_3 = tk.Radiobutton(app, variable=radioValue, value=2, text='Check M4_D0V_D1V_mode Die0 Pi Status 1000 times')
        rdio_4 = tk.Radiobutton(app, variable=radioValue, value=3, text='Check M4_D0V_D1V_mode Die0 Pi Status 1000 times')
        rdio_5 = tk.Radiobutton(app, variable=radioValue, value=4, text='Skip This Window')
    ...
    def THM_Value(self, THM_Voltage):
        temp_arr = [0.783926, 0.782122, 0.780318, 0.778512, 0.776705, 0.774896, 0.773085, 0.771273, 0.769460, 0.767645,
                            0.765828, 0.764010, 0.762190, 0.760369, 0.758546, 0.756722, 0.754896, 0.753068, 0.751239, 0.749408,
                            0.747576, 0.745742, 0.743907, 0.742070, 0.740232, 0.738392, 0.736550, 0.734707, 0.732863, 0.731017,
                            0.729169, 0.727320, 0.725469, 0.723617, 0.721763, 0.719908, 0.718051, 0.716192, 0.714333, 0.712471,
                            0.710608, 0.708744, 0.706878, 0.705011, 0.703142, 0.701272, 0.699400, 0.697527, 0.695652, 0.693776,
                            0.691899, 0.690020, 0.688139, 0.686258, 0.684374, 0.682490, 0.680604, 0.678716, 0.676827, 0.674937,
                            0.673046, 0.671153, 0.669258, 0.667363, 0.665466, 0.663568, 0.661668, 0.659767, 0.657865, 0.655961,
                            0.654056, 0.652150, 0.650243, 0.648334, 0.646424, 0.644513, 0.642601, 0.640687, 0.638772, 0.636856,
                            0.634939, 0.633020, 0.631100, 0.629179, 0.627257, 0.625334, 0.623410, 0.621484, 0.619558, 0.617630,
                            0.615701, 0.613771, 0.611839, 0.609907, 0.607974, 0.606039, 0.604104, 0.602167, 0.600230, 0.598291,
                            0.596351, 0.594410, 0.592468, 0.590526, 0.588582, 0.586637, 0.584691, 0.582744, 0.580796, 0.578848,
                            0.576898, 0.574947, 0.572995, 0.571043, 0.569089, 0.567135, 0.565179, 0.563223, 0.561265, 0.559307,
                            0.557348, 0.555388, 0.553427, 0.551466, 0.549503, 0.547539, 0.545575, 0.543610, 0.541644, 0.539677,
                            0.537709, 0.535740, 0.533771, 0.531801, 0.529829, 0.527858, 0.525885, 0.523911, 0.521937, 0.519962,
                            0.517986, 0.516009, 0.514032, 0.512053, 0.510074, 0.508094, 0.506114, 0.504132, 0.502150, 0.500167,
                            0.498183, 0.496199, 0.494214, 0.492228, 0.490241, 0.488253, 0.486265, 0.484276, 0.482287, 0.480296,
                            0.478305, 0.476313, 0.474320, 0.472327, 0.470333, 0.468338, 0.466342, 0.464346, 0.462349, 0.460351,
                            0.458353, 0.456354, 0.454354, 0.452353, 0.450352, 0.448350, 0.446347, 0.444344, 0.442339, 0.440335,
                            0.438329, 0.436322, 0.434315, 0.432308, 0.430299, 0.428290, 0.426280, 0.424269, 0.422258, 0.420246,
                            0.418233, 0.416219, 0.414205, 0.412190, 0.410174, 0.408158, 0.406141, 0.404123, 0.402104, 0.400085,
                            0.398065]
    ...
    def prtn_tca_unit_reg_cfg(self, block_idx, unit_reg_addr, data):
        ...
        config_data = (config_data | (block_idx << 22) | (unit_reg_addr & 0x1f) << 17 | (data & 0xffff))
        self.prtn_reg_write(0x48, config_data)  # command config
        self.prtn_info('Writing to TCA unit config_reg '+ str(unit_reg_addr)  +' val = ' + hex(config_data))


    def prtn_tca_internal_reg_cfg(self,block_idx, tca_inter_reg_addr, tca_inter_reg_data):
        self.prtn_tca_unit_reg_cfg(block_idx, 10, ((tca_inter_reg_addr & 0x1f) << 10) | 0x3ff)
    ...
    def prtn_qdca_osc_cfg(self, block_idx,include_dly_line, base_delay , fine_delay):
        ...
        tca_inter_reg_data = ( 0x1 << 15 | (include_dly_line & 0x1) << 14 | 0x3 << 9 | (fine_delay & 0xf) << 5 |  (base_delay & 0x1f))
        self.prtn_tca_internal_reg_cfg(block_idx, tca_inter_reg_addr, tca_inter_reg_data)
    ...
    def prtn_config_block(self, block_idx, base_delay, EW):
        ...
        tca_inter_reg_data = (0xf << 4) | (M1_edge_0 & 0x3) << 8 | (M1_edge_1 & 0x3) << 10
        self.prtn_tca_internal_reg_cfg(block_idx, tca_inter_reg_addr, tca_inter_reg_data)

        tca_inter_reg_addr = 4
        tca_inter_reg_data = (0xf << 4) | (M2_edge_0 & 0x3) << 8 | (M2_edge_1 & 0x3) << 10
        self.prtn_tca_internal_reg_cfg(block_idx, tca_inter_reg_addr, tca_inter_reg_data)
    ...
    def prtn_read_data(self, expected_count, expected_wait, unit_id):
        ...
        for idx in range(len(expected_count)):
            ...
            fifo_cnt = (int(self.prtn_reg_read(self.prtn_fifo_count_address), 0) >> 6) & 0x3f

            if (fifo_cnt != expected_count[idx]):
                self.prtn_error("Expected " + str(expected_count[idx]) + " entries in fifo , received " + str(fifo_cnt))
            else:
                self.prtn_info("Expected and got " + str(expected_count[idx]) + " entries in fifo")
    ...
    def prtn_info(self, msg):
        sys.stdout.write("PRTN_INFO (" + str(datetime.datetime.now()) + "): " + msg + '\n')
        sys.stdout.flush()

    def prtn_error(self, msg):
        sys.stdout.write("PRTN_ERROR (" + str(datetime.datetime.now()) + "): " + msg + '\n')
        sys.stdout.flush()
    ...
    def proteantecs(self, mode):
        ...
        block_idx_range = range(4)  # 42*4=168, S0:42,S1:42,S2:42,S3:42,S4:42,S5:42,S6:42,S7:42
        #block_idx_range = [0]
        ...
        cfg_range = [0x1e , 0xe, 0x5, 0x2 ,0x1, 0x0] #1_1110, 01110, 00110, 00010, 00001, 00000
                ...
                for EW in EW_range:
                    for qdca_osc in qdca_osc_bypass_cfg_range:
                        ...
                        for block_idx in block_idx_range:
                            ...
                            self.prtn_qdca_osc_cfg(block_idx=block_idx,include_dly_line=qdca_osc[0],base_delay=cfg,fine_delay=qdca_osc[1])
                        # for block_idx in block_idx_range:
