...
class Pico:
    ...
    def __init__(self, i2c_address) -> None:  # 7-bit slave address
        ...
        self.pyb.exec('i2c = I2C(1, sda=Pin('
                      +str(sda)+'), scl=Pin('
                      # +str(scl)+'), freq=1000_000)') # EY0008A okay
                      +str(scl)+'), freq=1000_000)')
        self.scan()
    def close(self) -> None:
        self.pyb.close()
    ...
    def write_bytes(self, slave, offset, val, bytes=4) -> None:
        self.pyb.exec('i2c.writeto_mem('
                      +str(slave) + ','
                      +str(offset)+ ','
                      +str(val.to_bytes(bytes, 'little'))+ ')' )
    def read_bytes(self, slave, offset, bytes=4) -> int:
        result = int(self.pyb.eval('int.from_bytes(i2c.readfrom_mem('
                  +str(slave) + ','
                  +str(offset)+ ','
                  +str(bytes) + '),\'little\')' ))
        return(result)
    def write(self, slave, offset, start_bit, field_size, val) -> None:
        ...
        if (start_bit == 0) and (field_size == 32):
            ...
            self.write_bytes(slave, offset, self.apply_bits(self.read_bytes(slave,offset), start_bit, field_size, val) )
        # self.GP25_low()
    def read(self, slave, offset, start_bit, field_size) -> hex:
        ...
        if (start_bit == 0) and (field_size == 32):
            ...
            rd_data = self.get_bits(self.read_bytes(slave, offset), start_bit, field_size)
            return hex(rd_data)
        # self.GP25_low()
