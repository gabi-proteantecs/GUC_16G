...
class Graph:
    ...
    def Word_start_head(self):
        ...
        doc.add_paragraph().add_run('Table X.X.X : ' + Intreface_name + TestFunction_val + '\n\n').bold = True
    ...
    def Word_table(self,num):
        ...
        # print(TestSpec_val)
        row_num = TestItem_number + 4  # Test item + table title(ex:NV - NT) + Spec. Max + Spec. Min
        # print(TestItem_number)
        cols_num = 7

        # edit test condition
        table = doc.add_table(rows=row_num, cols=cols_num, style='Table Grid')  # 创建带边框的表格
        table.cell(0, 0).paragraphs[0].add_run('').bold = True
        table.cell(0, 1).paragraphs[0].add_run('LV - LT').bold = True  #  table.cell(row, cols)
        ...
        if 'to' in TestSpec_val:  # Have "to" in TestSpec_val?
            for i in range(TestItem_number):
                ...
                table.cell(i+1, 0).paragraphs[0].add_run(Sweep_Array_1Darray[i]).bold = True  # test chip corsor/number table title
                table.cell(i+1, 6).paragraphs[0].add_run(TestUnit_val).bold = True  # test chip 'Unit' table title #  table.cell(row, cols)
            table.cell(i+2, 0).paragraphs[0].add_run('Spec. Min').bold = True  # Spec. Max #  table.cell(row, cols)
            table.cell(i+2, 6).paragraphs[0].add_run(TestUnit_val).bold = True  # test chip 'Unit' table title #  table.cell(row, cols)
            table.cell(i+3, 0).paragraphs[0].add_run('Spec. Max').bold = True # Spec. Min #  table.cell(row, cols)
            table.cell(i+3, 6).paragraphs[0].add_run(TestUnit_val).bold = True  # test chip 'Unit' table title #  table.cell(row, cols)
            for col in range(5):
                SpecRange = TestSpec_val.split('to')
                table.cell(TestItem_number+1, col+1).paragraphs[0].add_run(SpecRange[0]).bold = False
                SpecMax = SpecRange[1].strip("\n")
                # print(SpecMax)
                table.cell(TestItem_number+2, col+1).paragraphs[0].add_run(SpecMax).bold = False
            ...
            for i in range(TestItem_number):
                ...
                table.cell(i+1, 0).paragraphs[0].add_run(Sweep_Array_1Darray[i]).bold = True  # test chip corsor/number table title
                table.cell(i+1, 6).paragraphs[0].add_run(TestUnit_val).bold = True  # test chip 'Unit' table title #  table.cell(row, cols)
            table.cell(i+2, 0).paragraphs[0].add_run('Spec. Max').bold = True  # Spec. Max #  table.cell(row, cols)

            table.cell(i+2, 6).paragraphs[0].add_run(TestUnit_val).bold = True  # test chip 'Unit' table title #  table.cell(row, cols)
            for col in range(5):
                SpecMax = TestSpec_val.strip("\n")
                table.cell(TestItem_number + 1, col + 1).paragraphs[0].add_run(SpecMax).bold = False
            for i in range(TestItem_number + 1):  # test item num + spec. Max
                table.rows[i + 1].height = Pt(25)
            ...
            for row in range(TestItem_number):
                ...
                for i in range(999):
                    try:
                        ...
                        # print(DataRate_val)
                        if s.find(condition_val) == -1: # Check LV - LT , HV - LT , NV - NT , LV - HT , HV - HT
                            pass
                        else:
                            # print('value',s)
                            if s.find(Sweep_val) == -1: # check test item number
                                ...
                                if DataRate_val != TestDataRate_val:
                                    ...
                                    # print(my_array_val)
                                    table.cell(row + 1, col + 1).paragraphs[0].add_run(my_array_val)  # table.cell(row, cols) Start write test value
                                    doc.save(r"Test Report.docx")
                    except Exception:
                        break
    ...
    def Word_picture(self):
        ...
        last_paragraph = doc.paragraphs[-1]  # 段落屬性，在這裏代表每一行，一共三行，-1為最後一行
        ...
        paragraph.add_run('Table X.X.X : ' + Intreface_name + TestFunction_val + '\n\n').bold = True
    ...
    def xls_ipxact(self,**kargs):
        ...
        if reg_source == 'xlsx':
            ...
            for i in range(row_E - row_S):
                ...
                if offset == 'skip'  or offset == 'Offset_Address' or (str(offset).strip()) == 'None':
                    pass
                else:
    ...
    def txt_log_check(self,**kargs):
            ...
            for log in Chip_Info_list:
                ...
                if buffer1 == -1 and buffer2 == -1:
                    ...
                    for i in range(40):
                        ...
                        if T_F3 != -1:
                            fail_lane_num = (Lan_Fail.split('Total Lane Fail NUM = '))[1]
                    ...
                    if slices == '':
                        if err_chk_fail == 1:
                            fail_str = err_chk_info.strip() + '_' + log.strip() + '_ERRFAIL'
                        elif vco_fail == 0:
                        ...
                        fail_str = (pattern.strip() + '_' + log.strip() + '_' + slices.strip() +
                                    fail_lane_num + rst_fail_num)
                    # print(fail_str)
                    fail_list += [fail_str]
                index += 1
        ...
        chk_list = ['PRBS31', 'PRBS7', 'CLOCK', 'All-0', 'All-1', 'USER0_b15b0', 'PRBS32', 'P0P1']
        chk_list = ['PRBS9', 'PRBS7', 'CLOCK', 'All-0', 'PRBS31', 'USER0_b15b0', 'PRBS32', 'P0P1']
        F_PAT_list = []
        BIST_FN = []
        BIST_RstFN = []
        for ckp in chk_list:
            ...
            for i in range(len(fail_list)):
                ...
                if T_F != -1:
                    if fail_list[i].find("PASS") == -1:
                        ...
                        if fail_list[i].find("LOL") == -1 and fail_list[i].find("ERRFAIL") == -1:
        ...
        with open(txt_path, "r") as f:
            ...
            for log in Chip_Info_list:
                ...
                if buffer1 == -1:
                    ...
                    counter_info = [Chip_Info_list[index + 1], Chip_Info_list[index + 2]]
                    counter_value_ck = counter_info[0].split(',') + counter_info[1].split(',')
                    cntv = []
            ...
            for k in range(len(die_list)):
                for i in range(8):
                    if chk_idx > len(sort_cnt):
                        print(f'check index {chk_idx} > {len(sort_cnt)}, not expected value')
    ...
    def txt_log_hwt_check(self,**kargs):
        ...
        def hwt_1t_check(table, init = 0):
            if init == 1:
                ...
                for i in range(8):
                    ...
                    v_cen1_list += [v_cen1]
            HW_1T_list = [left1_list, right1_list, center1_list, win1_list,
                          low1_list, high1_list,v_cen1_list, v_height1_list]
        ...
        def hwt_check(table):
            ...
            if len(check_table) > 2:
                ...
                [left1_list, right1_list, center1_list, win1_list, low1_list, high1_list,
                 v_cen1_list, v_height1_list] = hwt_1t_check(table_1)
                [left2_list, right2_list, center2_list, win2_list, low2_list, high2_list,
                 v_cen2_list, v_height2_list] = hwt_1t_check(table_2)
            elif len(check_table) > 1:
                if check_table[0].find('train results: RX'):
                    [left1_list, right1_list, center1_list, win1_list, low1_list, high1_list,
                     v_cen1_list, v_height1_list] = hwt_1t_check(check_table[1], 1)
                    [left2_list, right2_list, center2_list, win2_list, low2_list, high2_list,
                     v_cen2_list, v_height2_list] = hwt_1t_check(check_table[1])
                elif check_table[0].find('train results: TX'):
                    [left1_list, right1_list, center1_list, win1_list, low1_list, high1_list,
                     v_cen1_list, v_height1_list] = hwt_1t_check(check_table[1])
                    [left2_list, right2_list, center2_list, win2_list, low2_list, high2_list,
                     v_cen2_list, v_height2_list] = hwt_1t_check(check_table[1], 1)
                else:
                    print(f'\033 no find expected training results set tab to -1')
                    [left1_list, right1_list, center1_list, win1_list, low1_list, high1_list,
                     v_cen1_list, v_height1_list] = hwt_1t_check(check_table[0], 1)
                    [left2_list, right2_list, center2_list, win2_list, low2_list, high2_list,
                     v_cen2_list, v_height2_list] = hwt_1t_check(check_table[0], 1)
            else:
                print(f'\033 no find expected training results set tab to -1')
                [left1_list, right1_list, center1_list, win1_list, low1_list, high1_list,
                 v_cen1_list, v_height1_list] = hwt_1t_check(check_table[0], 1)
                [left2_list, right2_list, center2_list, win2_list, low2_list, high2_list,
                 v_cen2_list, v_height2_list] = hwt_1t_check(check_table[0], 1)
            HW_Table = (left1_list + right1_list + left2_list + right2_list +
                        center1_list + win1_list + center2_list + win2_list +
                        low1_list + high1_list + low2_list + high2_list +
                        v_cen1_list + v_height1_list + v_cen2_list + v_height2_list)
            return HW_Table
    ...
    def txt_log_full_eye_check(self,**kargs):
            ...
            for each_eye_t, each_off, each_eye_info in zip(table, offset, eye_info):
                ...
                for k in range(32):
                    try:
                        ...
                        if simple == 1:
                            Signal_Eye_Table[2][k] = -1 if each_line[k].find('1') < 0 else 63 - each_line[k].find('1')
                            ...
                            for j in range(64):
                                ...
                                Signal_Eye_Table[4][k][eye_p] = int(each_line[k][63 - j])
                                if j == 0:
                                    elc = j if each_line[k][63 - j] == '1' else e_l
                                    erc = j if each_line[k][63 - j] == '1' else e_r
                                else:
                                    elc = j if each_line[k][63 - j] == '1' and each_line[k][64 - j] == '0' else elc
                ...
                if mode == 1:
                    if Signal_Eye_Table[2][ck_idx] > -100:
                        Signal_Eye_Table[3][0] = Signal_Eye_Table[2][ck_idx] - Signal_Eye_Table[1][ck_idx] + 1
                        Signal_Eye_Table[3][2] = int((Signal_Eye_Table[2][ck_idx] + Signal_Eye_Table[1][ck_idx]) / 2)
                    else:
                    ...
                    for j in range(32):
                        if j == 0:
                            ...
                            ebc = j if Signal_Eye_Table[4][j][32] >= 1 and Signal_Eye_Table[4][j-1][32] == 0 else ebc
                ...
                Eye_Table.append([Signal_Eye_Table[0], Signal_Eye_Table[1].copy(), Signal_Eye_Table[2].copy(),
                                  Signal_Eye_Table[3].copy(), copy.deepcopy(Signal_Eye_Table[4])])
            return Eye_Table
        ...
        if len(table.split('eye results:')) > 1:
            ...
            if offset_apply == 1:
                ...
                Eye_Table = eye_check(table_eye, [0] * len(offset_list), eye_info_list, mode)
        else:
    ...
    def Graph_TX_Eye_and_Jitter(self):
        ...
        for i in range(1):
            try:
                ...
                if 'to' in TestSpec_val:
                    ...
                    x = ['LV - LT', 'HV - LT', 'NV - NT', 'LV - HT', 'HV - HT']
                    plt.plot(x, Specification_Max, 'D--', label='Spec. Min', linewidth=3, markersize=8, c='r')
                    plt.plot(x, Specification_Min, 'D--', label='Spec. Max', linewidth=3, markersize=8, c='r')
                    ...
                    Specification = List
                    x = ['LV - LT', 'HV - LT', 'NV - NT', 'LV - HT', 'HV - HT']
                    plt.plot(x, Specification, 'D--', label='Spec. Max', linewidth=3, markersize=8, c='r')
                else:
                    ...
                    Specification = List
                    x = ['LV - LT', 'HV - LT', 'NV - NT', 'LV - HT', 'HV - HT']
                    plt.plot(x, Specification, 'D--', label='Spec. Min', linewidth=3, markersize=8, c='r')
            except Exception:
                break

        font = {'family': 'Arial', 'weight': 'normal', 'size': 10}
        plt.title(Intreface_name + TestFunction_val +'\n', font, fontsize=Graph_ScaleSize)
        # plt.xlabel("XX")
        ...
        plt.legend(loc='upper left', bbox_to_anchor= (1,1), frameon=False, borderaxespad=0, fontsize=Graph_ScaleSize)
        ...
        img = cv.copyMakeBorder(img, 1, 1, 1, 1, cv.BORDER_CONSTANT, value=[1, 1, 1])  # 添加边框
    ...
    def Graph_kvco(self,**kargs):
        ...
        for i in range(txt_lines_num):   # for i in range(txt_lines_num):
            list0 = text[i].split(",")
            # print(list0)
            Color_array = ['b', 'c', 'k', 'g', 'm', 'r','y','tab:blue', 'tab:orange', 'tab:green', 'tab:red', 'tab:purple', 'tab:brown', 'tab:pink', 'tab:gray', 'tab:olive', 'tab:cyan',
                           'b', 'c', 'k', 'g', 'm', 'r','y','tab:blue', 'tab:orange', 'tab:green', 'tab:red', 'tab:purple', 'tab:brown', 'tab:pink', 'tab:gray', 'tab:olive', 'tab:cyan',
                           'b', 'c', 'k', 'g', 'm', 'r','y','tab:blue', 'tab:orange', 'tab:green', 'tab:red', 'tab:purple', 'tab:brown', 'tab:pink', 'tab:gray', 'tab:olive', 'tab:cyan',
                           'b', 'c', 'k', 'g', 'm', 'r','y','tab:blue', 'tab:orange', 'tab:green', 'tab:red', 'tab:purple', 'tab:brown', 'tab:pink', 'tab:gray', 'tab:olive', 'tab:cyan',
                           'b', 'c', 'k', 'g', 'm', 'r','y','tab:blue', 'tab:orange', 'tab:green', 'tab:red', 'tab:purple', 'tab:brown', 'tab:pink', 'tab:gray', 'tab:olive', 'tab:cyan',
                           'b', 'c', 'k', 'g', 'm', 'r','y','tab:blue', 'tab:orange', 'tab:green', 'tab:red', 'tab:purple', 'tab:brown', 'tab:pink', 'tab:gray', 'tab:olive', 'tab:cyan',
            ...
            plt.plot(List1, List2, 'o-', label = kvco_line_name, linewidth=1, markersize=3, c = Line_color)
        ...
        plt.legend(bbox_to_anchor=(1.01, 1), loc='upper left', borderaxespad=0.) # Show note lable
    ...
    def Graph_slewRate(self):
        ...
        for csv in range (len(csv_list)-1):
            try:
                # print(csv)
                graph_color_array = ['k', 'r', 'c', 'b', 'g', 'c', 'm', 'y', 'tan'] # define graph line color
                ...
                # print(Line_name)
                plt.plot(List1, List2, '-', label=Graph_lable_name, linewidth=1, markersize=3, c =graph_color_array[csv+1])
            except Exception:
                break
        ...
        labels = ax.get_xticklabels() + ax.get_yticklabels()
        [label.set_fontname('Arial') for label in labels]
        plt.legend(bbox_to_anchor=(1.01, 1), loc='upper left', borderaxespad=0.1)  # Show note lable
    ...
    def Graph_shmoo_voltage(self, **kargs):
        ...
        for data, color, group in zip(data, colors, groups):
            x, y = data
            plt.scatter(x, y, alpha=1, c=color, edgecolors='black', s=250, label=group, marker='s')
    ...
    def Graph_shmoo_vref(self, **kargs):
        ...
        for z in range(6, sheet.max_row + 1):
            if sel_chip == sheet.cell(row=z, column=chip_col).value and \
                    sel_mode == sheet.cell(row=z, column=mode_col).value and \
                    sel_temper == str(sheet.cell(row=z, column=temper_col).value):
        ...
        for slice_sel in ['Die1-Slice-0', 'Die1-Slice-1', 'Die1-Slice-2', 'Die1-Slice-3', 'Die1-Slice-4',
                          'Die1-Slice-5', 'Die1-Slice-6', 'Die1-Slice-7', 'Die0-Slice-0', 'Die0-Slice-1',
                          'Die0-Slice-2', 'Die0-Slice-3', 'Die0-Slice-4', 'Die0-Slice-5', 'Die0-Slice-6',
                          'Die0-Slice-7']:
            ...
            self.Graph_shmoo_vref_plot(check_row=check_row, sheet=sheet, sel_avdd=sel_avdd,
                                       vref_list=vref_list, slice_num=slice_num, path=path,
                                       Title=Title, f_Title=f_Title)
            ...
            for row in range(ws.max_row + 1 - start_row):
                ...
                if sel_mode != "":
                    if sheet.cell(row=chk_row, column=mode_col).value != sel_mode:
                        continue
                if sel_temper != "":
                    if str(sheet.cell(row=chk_row, column=temper_col).value) != sel_temper:
                        continue
                if sel_avdd != "":
                    if not sheet.cell(row=chk_row, column=avdd_col).value:
                        continue
                    if float(sel_avdd) != float(sheet.cell(row=chk_row, column=avdd_col).value):
                        continue
                ...
                if path_val != None:
                    ...
                    Full_Eye_Results_list = self.txt_log_full_eye_check(txt_path=txt_path, mode=detail_m,
                                                                        off_aply=off_aply)
                else:
                ...
                HYPERLINK = "=HYPERLINK(\"" + rel_sp + "\"," + "\"" + "Fig_Folder" + "\"" + ")"
                ...
                for one_eye_data in Full_Eye_Results_list:
                    ...
                    slice_num = slice_n
                    self.Graph_shmoo_vref_plot(train_mode=1+detail_m, sheet=sheet, sel_avdd=float(avdd_get),
                                               vref_list=range(0, 64, 2), slice_num=int(slice_num), path=sp,
                                               Title=Title, f_Title=f_Title, train_left=one_eye_data[1],
                                               train_right=one_eye_data[2], eye_detail=one_eye_data[4])
                    cn += 1
                    ...
                    HYPERLINK = "=HYPERLINK(\"" + rel_sp + 'Vref Window_' + f_Title + '.png' + "\"," + "\"" + \
    ...
    def Graph_shmoo_vref_BK(self, **kargs):
        ...
        # "win_left Excel's col num"_"Die and slice name"
        for slice in ['19_Die0-Slice-1', '20_Die0-Slice-2', '21_Die0-Slice-3', '22_Die0-Slice-4', '23_Die0-Slice-5',
                      '24_Die0-Slice-6','25_Die0-Slice-7', '26_Die0-Slice-8', '35_Die1-Slice-1', '36_Die1-Slice-2',
                      '37_Die1-Slice-3','38_Die1-Slice-4','39_Die1-Slice-5', '40_Die1-Slice-6', '41_Die1-Slice-7',
                      '42_Die1-Slice-8']:
            ...
            for i in range(Reg_Num):
                Win_L = (int(sheet.cell(row=i + 6, column=slice_num).value) - 32) / 64  # -31 is set_bar_center lable need 0
                Win_R = (int(sheet.cell(row=i + 6, column=slice_num + 8).value) - 32) / 64  # -31 is set_bar_center lable need 0
                if Win_L >= Win_R:
                    gnt.broken_barh([(-1, 99)], (10 * (i + 1), 10), facecolors='tab:red')
                else:
                    gnt.broken_barh([(-32 / 64, (abs(-32 / 64 - (Win_L))))], (10 * (i + 1), 10), facecolors='tab:red')
                    gnt.broken_barh([(Win_L, (abs(Win_R - Win_L)))], (10 * (i + 1), 10), facecolors='tab:green')
                    gnt.broken_barh([(Win_R, 99)], (10 * (i + 1), 10), facecolors='tab:red')
                if Win_L == None:
                    break
    ...
    def Graph_shmoo_vref_sheet(self, **kargs):   #Manual sheet use
        ...
        if len(check_row) == 0:
            ...
            return
        # "win_left Excel's col num"_"Die and slice name"
        for slice_sel in ['DieA-Slice-0', 'DieA-Slice-1', 'DieA-Slice-2', 'DieA-Slice-3', 'DieA-Slice-4',
                          'DieA-Slice-5', 'DieA-Slice-6', 'DieA-Slice-7', 'DieB-Slice-0', 'DieB-Slice-1',
                          'DieB-Slice-2', 'DieB-Slice-3', 'DieB-Slice-4', 'DieB-Slice-5', 'DieB-Slice-6',
                          'DieB-Slice-7']:
            slice_n = int((slice_sel.split('-'))[2])
            ...
            self.Graph_shmoo_vref_plot(check_row=check_row, sheet=sheet, sel_avdd=sel_avdd,
                                       vref_list=vref_list, slice_num=slice_num, path=path,
                                       Title=Title, f_Title=Title, ytick_mod=1)
            # # figure = plt.gcf()
            # figure = plt.figure()
            # fig, gnt = plt.subplots()  # Declaring a figure "gnt"
            # gnt.set_ylim(5, 45)  # Setting Y-axis limits
            ...
            for i in range(32):
                ...
                for k in range(xp):
                    ...
                    if eye_detail[i][k] == max(max(eye_detail)):
                        ...
                        plot_f = 'y'
                    gnt.broken_barh([((k - xp/2) / xp, 1 / xp)], (10 * i_idx, 10), facecolors=plot_f)

        elif sheet is not None:
            i_idx = 0
            for i in check_row:
                i_idx = i_idx + 1  # -6 to get correct idx (5 rows for title)
                Win_L = (int(sheet.cell(row=i, column=slice_num).value) - 31) / 64  # -31 is set_bar_center lable need 0
                Win_R = (int(sheet.cell(row=i, column=slice_num + 8).value) - 31) / 64  # -31 is set_bar_center lable need 0
                if Win_L >= Win_R:
                    gnt.broken_barh([(-1, 99)], (10 * i_idx, 10), facecolors='tab:red')
                else:
                    gnt.broken_barh([(-32 / 64, (abs(-32 / 64 - Win_L)))], (10 * i_idx, 10), facecolors='tab:red')
