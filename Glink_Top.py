...
class MainFrame(gui.MainFrame):
    ...
    def connect(self, event):
        ...
        self.eye_graph_info_wx.SetBitmap(wx.Bitmap( u"TestTools/blank.png", wx.BITMAP_TYPE_ANY))
                    ...
                    if Seach_temp == 0:
                        ...
                        if self.ThermalOn_OFF.Value:
                            ...
                            for i in range(Temp_Delay):
                                ...
                                print('Temperature Delay :', + (Temp_Delay - i), flush=True)
                        ...
                        for R in range(sheet.max_row):
                            ...
                            self.Item_Temp_value = sheet.cell(row=row_s+R, column=3).value
                            if self.Test_OnOff == 'ON' and self.Temp_now == self.Item_Temp_value:
                                ...
                                print('Test Temperature : ', self.Temp_now, 'Degree')
                                self.chip_version = str(self.Chip_Corner) + '_' + str(self.chip_number)
                                print(f'PVT Corner : {self.chip_version}')
                                self.TestDataRate = sheet.cell(row=row_s + R, column=4).value
                                print(f'Test DataRate : {self.TestDataRate}')

                                # for D2D/UCIe Test
                                if (self.TestItem).find('Specialized') == -1:
                                    self.mode_name = sheet.cell(row=row_s + R, column=5).value
                                    self.Chip_Mode = self.mode_name
                                    print(f'Test Mode : {self.Chip_Mode}')
                                    worst_function = ['Seach Eye High Min', 'Seach Eye Width Min',
                                                      'Seach Phase Fail Max', 'Merge All Slice']
                                    self.eye_scan_type.Items = ['Die2_S0', 'Die2_S2', 'Die0_S0',
                                                                'Die0_S2'] + worst_function  # by chip
                                    self.eye_scan_type.Selection = 0  # by chip
                                    if sheet.cell(row=row_s + R, column=6).value == 'ON':
                                        self.Training_set = 'Enable'
                                    else:
                                        self.Training_set = 'Disable'
                                    print(f'Hardware Training : {self.Training_set}')
                                    rst_list = (
                                    'Hardware Reset (GPIO_Reset)', 'Hardware Reset (Power_Reset)', 'Skip Reset')
                                    self.sys_rst_num = sheet.cell(row=row_s + R, column=7).value
                                    print(f'Chip Reset : {rst_list[self.sys_rst_num]}')
                                    self.BIST_loop = sheet.cell(row=row_s + R, column=8).value
                                    self.BIST_time = sheet.cell(row=row_s + R, column=9).value
                                    print(f'BIST Test Loop : {self.BIST_loop} / BIST Test Time : {self.BIST_time}')
                                    self.map_col_name = sheet.cell(row=row_s + R, column=10).value
                                    self.register_setup_name = sheet.cell(row=row_s + R, column=11).value
                                    print(f'Excel Register Setup Sheet Seaunece Name : {self.register_setup_name}')
                                    self.File_name = sheet.cell(row=row_s + R, column=12).value
                                    if self.File_name == None:
                                        self.File_name = ''
                                        print(f'FileName : None')
                                    else:
                                        print(f'FileName : {self.File_name}')
                                    self.note = sheet.cell(row=row_s + R, column=13).value
                                    ...
                                    all_power_en = sheet.cell(row=row_s + R, column=30).value
                                    if all_power_en != 'NA' and all_power_en != 0 and all_power_en != None :
                                        # use all power banana connect
                                        power1 = sheet.cell(row=row_s + R, column=31).value
                                        power2 = sheet.cell(row=row_s + R, column=32).value
                                        self.visa.E36233A_setup(power1,power2,'20','20')
                                        self.visa.E36233A_Out_ON_RST()
                                        time.sleep(2)
                                    else:
                                        pass
                                    for num in range(10):
                                        command = (sheet.cell(row=row_s + R, column=14 + num).value).upper()
                                        if command == 'BYPASS':
                                            pass
                                        elif command != None:
                                            self.power_source_autotest(command=command, num=num)
                                        else:
                                            pass
                                        if self.avdd_sense_en == 1:
                                            self.voltage_sense_avdd = (((sheet.cell(row=row_s + R, column=14 + 4).value).upper()).split('/'))[1]
                                        else:
                                    ...
                                    even_list = []
                                    for e in range(20):
                                        even = sheet.cell(row_s + R, column=34 + e).value
                                    ...
                                    self.even_4 = even_list[3] # pcs bist check pi cycles
                                    self.even_5 = even_list[4] # scan pi satus , after HW training
                                    self.even_6 = even_list[5] # pmad test pattern set
                                    self.even_7 = even_list[6] # sw pi range set
                                    self.even_8 = self.lane_valid_en = even_list[7] # Lane "valid" enable
                                    self.even_9 = self.sw_bist = even_list[8] # software training test - test mode select
                                    self.even_10 = even_list[9] # software training test 1 lane
                                    ...
                                    for s in range(51):
                                        buffer = sheet.cell(row_s + R, column=s + 3).value
    ...
    def Register_init(self):
        ...
        if self.reg_sequence_loops-1 == reg_col_num:
            print(f'\033The  " {self.register_setup_name} "  register sequence read failed')
        else:
            self.pll_en_reg = self.pll_seach_xls_str(df, 'PLL', 'HW1', reg_vai_num=reg_col_num)
            self.hw_non_1 = self.pll_seach_xls_str(df, 'HW1', 'PLL2', reg_vai_num=reg_col_num)
            self.PLL_NLB = self.pll_seach_xls_str(df, 'PLL2', 'HW2', reg_vai_num=reg_col_num)
            self.hw_NLB = self.pll_seach_xls_str(df, 'HW2', 'PCS1', reg_vai_num=reg_col_num)
            self.PCS_BIST_Check_NON = self.pll_seach_xls_str(df, 'PCS1', 'PMAD1', reg_vai_num=reg_col_num)
            self.PMAD_BIST_Check_NON = self.pll_seach_xls_str(df, 'PMAD1', 'USER1', reg_vai_num=reg_col_num)
            self.User_Define_Squence_1 = self.pll_seach_xls_str(df, 'USER1', 'USER2', reg_vai_num=reg_col_num)
            self.User_Define_Squence_2 = self.pll_seach_xls_str(df, 'USER2', 'DONE', reg_vai_num=reg_col_num)
    ...
    def TestItem_Init(self, **kwargs):
        ...
        if TestItem == 'ABP_Enable':
            ...
            self.Test_Info_list = [str(self.pass_fail), self.pll_LOL, 'NA', 'NA', 'NA'] # file_name / PASS or FAIL / HW Training / PMAD / PCS
            ...
            if self.eye1D2D == 'NA' :
                ...
                self.ChkLog_fail(find='failed')
                self.Test_Info_list = [str(self.pass_fail), self.pll_LOL, self.PASS_FAIL_HW_chk(), 'NA', 'NA'] # file_name / PASS or FAIL / HW Training / PMAD / PCS
        else:
            pass
        ...
        self.TestItem_Now2_wx.Value = f"{self.TestItem_full} , \nTest Result : {self.pass_fail}"
    ...
    def HW_Training_init(self):
        ...
        self.slice_result = self.run_0.Hardware_Training_Non(
                                    ...
                                    hw_non_1=self.hw_non_1, setup_lane=self.even_6,
                                    vref_start=self.vref_start,
                                    log_type=self.log_type, eye_scan=self.eye_scan)
        self.data_replay = 0
        self.bist = self.run_0.PCS_BIST_Check_NON(mode='M4_D0V_D1V_mode', PCS_BIST_Check_NON=self.PCS_BIST_Check_NON,
                                                chk_loop=self.BIST_loop, chk_time=self.BIST_time, data_replay=self.data_replay,
                                                voltage_sense_avdd=self.voltage_sense_avdd, avdd_sense_en=self.avdd_sense_en)
        ...
        self.slice_result = self.run_0.Hardware_Training_Non(
                                    ...
                                    data_training_en=data_training_en,
                                    hw_non_1=self.hw_non_1, setup_lane=self.even_6,
                                    vref_start=self.vref_start, lane_set_arr=self.lane_set_arr,
                                    log_type=self.log_type, eye_scan=self.eye_scan)
        self.data_replay = 0
        self.bist = self.run_0.PCS_BIST_Check_NON(mode='M4_D1H_D2V_mode', PCS_BIST_Check_NON=self.PCS_BIST_Check_NON,
                                                chk_loop=self.BIST_loop, chk_time=self.BIST_time, data_replay=self.data_replay,
                                                voltage_sense_avdd=self.voltage_sense_avdd, avdd_sense_en=self.avdd_sense_en)
    ...
    def SW_Training_init(self):
        ...
        if  self.sw_vref_type == 'All_H' :
            all_result = self.run_0.Software_Training_Non_H(
                                    ...
                                    bist_mode_select=bist_mode_select, set_vref=self.set_vref_now,
                                    pi_step_even=self.pi_step_even, vref_loop_now=self.vref_loop_now,
                                    sw_vref_type=self.sw_vref_type, chk_time=int(self.BIST_time),
                                    sw_vref_loops=self.sw_vref_loops, vref_en=self.vref_en,
                                    lane_valid_en=self.lane_valid_en, vref_bypass=self.vref_bypass,
            ...
            all_result = self.run_0.Software_Training_Non_W(
                                    ...
                                    sw_vref_loops=self.sw_vref_loops, vref_en=self.vref_en,
                                    lane_valid_en=self.lane_valid_en, vref_bypass=self.vref_bypass,
                                    sw_bist=self.sw_bist, voltage_sense_avdd=self.voltage_sense_avdd,
    ...
    def PCS_BIST_Check_NON_Path(self):
        self.bist = self.run_0.PCS_BIST_Check_NON(mode=self.Chip_Mode, PCS_BIST_Check_NON=self.PCS_BIST_Check_NON,
                                                chk_loop=self.BIST_loop, chk_time=self.BIST_time, data_replay=self.data_replay,
                                                voltage_sense_avdd=self.voltage_sense_avdd, avdd_sense_en=self.avdd_sense_en)
    def PMAD_BIST_Check_NON_Path(self):
        self.bist = self.run_0.PMAD_BIST_Check_NON( mode=self.Chip_Mode, PMAD_BIST_Check_NON=self.PMAD_BIST_Check_NON,
                                                                                        chk_time=self.BIST_time, chk_loop=self.BIST_loop,voltage_sense_avdd=self.voltage_sense_avdd,
                                                                                        avdd_sense_en=self.avdd_sense_en)
    def PMAD_BIST_Check_NON_Path_Loop1(self):
        self.bist = self.run_0.PMAD_BIST_Check_NON( mode=self.Chip_Mode, PMAD_BIST_Check_NON=self.PMAD_BIST_Check_NON,
                                                                                        chk_time=1, chk_loop=1, voltage_sense_avdd=self.voltage_sense_avdd,
                                                                                        avdd_sense_en=self.avdd_sense_en)
    ...
    def Start_Test(self, **kwargs):
        ...
        self.run_0.log_info(f'Test Tempature={self.Temp_now} Degree_DataRate={self.TestDataRate} Gb/s_Test Mode={self.Chip_Mode}')
        self.phy_0.log_info(f'Test Tempature={self.Temp_now} Degree_DataRate={self.TestDataRate} Gb/s_Test Mode={self.Chip_Mode}', reg_save)
        # clear log
    ...
    def log_name(self):
        ...
        if self.TestItem == 'VCO':
            self.Log_Folder_path =   self.TestItem_full + '_' + \
                                                str(self.Temp_now) + 'Degree_' + self.chip_version + '_' + \
                                                str(self.TestDataRate) + 'Gbps_' + \
                                                str(self.Chip_Mode) + '_' + \
                                                str(self.eye_scan) + '_' + \
                                                str(self.File_name) +  '_' +str(Loop_time)
        else:
        ...
        if self.eye1D2D != 'NA':
            for n in range(12):
                ...
                textfile.write(f'{self.Chip_Mode}\n{self.TestDataRate}\n{self.eye_txt_arr[n]}\n{self.save_log}')
                textfile.close()
    ...
    def slice_map_data(self):
        if self.slice_sheet_name_tree.GetValue() != '':
            ...
            self.map_print_en = 0
            slice_pmaa_map = self.read_xls_tree(map='slice', map_path=self.slice_map_path)
            self.slice_map_tree = self.read_slice_write_map_tree(slice_pmaa_map=slice_pmaa_map)
            buffer = 'Ready'
        else:
            # h = ctypes.windll.LoadLibrary("C:\\Windows\\System32\\user32.dll")
    ...
    def read_pll_write_map_tree(self, **kargs):
        ...
        for r in range(len(pll_pmaa_map)):
            reg_list = (pll_pmaa_map[r]).split(",")
            offset = hex(int(reg_list[2],16) + int(0x2000)) # pll regisyer start 0x2000
            bit = reg_list[3]
    ...
    def read_slice_write_map_tree(self, **kargs):
        ...
        for r in range(len(slice_pmaa_map)):
            ...
            offset = hex(int(reg_list[2], 16) + buffer)  # UCIe/Slice/PCS/Adapter regisyer start
            bit = reg_list[3]
            if reg_list[5].find('h') != -1:
                register_value = (reg_list[5]).split("h")
                register_value = int(register_value[1], 16)
            ...
            reg_value = f'{reg_list[1]},{offset},{bit},{register_value},{reg_list[4]},{type}'
            array += [reg_value]
        return array
    def i2c_read_compare(self):
        ...
        reg_arr = [f'{self.offset},{self.bit},nan,nan,nan,nan,v,{self.die},{self.group_sel_num},{self.slice}']
        if self.reg_type.GetValue() == 'Normal I2C':
            buffer = self.phy_0.normal_i2c(self.die, self.group_sel_num, self.offset_hex, self.bit_S, self.bit_Leng, 'read')
        elif self.reg_type.GetValue() == 'TPORT':
            buffer = self.phy_0.indirect_read(self.EHOST[self.die][0], self.offset_hex, self.bit_S, top=1)
        else:
            buffer = self.phy_0.reg_user_set(reg_arr=reg_arr, show=0, mode='gui_tree', gui_die_sel=self.die, gui_group_num=self.group_sel_num, gui_slice_num=self.slice) # PLL/Slice
        self.i2c_register_value.Value = self.offset_tree = self.compare_value = buffer
    def i2c_read(self, event):
        self.offset_ref = 'user'
        if len(self.i2c_bit_MSB.GetValue()) != 0 and len(self.i2c_bit_LSB.GetValue()) != 0 and len(self.i2c_offset.GetValue()) != 0:
            self.i2c_read_write()
            self.phy_0.die_sel(die=self.die)
            reg_arr = [f'{self.offset},{self.bit},nan,nan,nan,nan,v,{self.die},{self.group_sel_num},{self.slice}']
    ...
    def i2c_write(self, event):
        if len(self.i2c_bit_MSB.GetValue()) != 0 and len(self.i2c_bit_LSB.GetValue()) != 0 and len(self.i2c_offset.GetValue()) != 0:
            ...
            if self.reg_type.GetValue() == 'Normal I2C':
                self.phy_0.normal_i2c(self.die, self.group_sel_num, self.offset_hex, self.bit_S, self.bit_Leng, 'write', setv=self.register_value)
            elif self.reg_type.GetValue() == 'TPORT':
                self.phy_0.indirect_write(self.EHOST[self.die][0], self.offset_hex, self.bit_S, self.register_value, top=1)
            else:
                reg_arr = [f'{self.offset},{self.bit},{hex(self.register_value)},nan,nan,v,nan,{self.die},{self.group_sel_num},{self.slice}']
                self.phy_0.reg_user_set(reg_arr=reg_arr, show=0, mode='gui_tree', gui_die_sel=self.die, gui_group_num=self.group_sel_num, gui_slice_num=self.slice) # PLL/Slice)
    def i2c_read_write(self):
        ...
        if self.i2c_bit_MSB.GetValue() == self.i2c_bit_LSB.GetValue():
            ...
            self.bit_Leng = int((int(self.i2c_bit_MSB.GetValue()) - int(self.i2c_bit_LSB.GetValue())) + 1)
    ...
    def power_update_even(self, even):
        power_arr = [self.power_select1.GetValue(),
        ...
        value_arr = [self.power_value1.GetValue(),
        ...
        current_arr = [self.power_current1.GetValue(),
        ...
        for num in range(10):
            ...
            if buffer.find('PMIC') != -1:
                ...
                command = "36000"+ '/' + (buffer_list[1])[6] +  '/' + (buffer_list[2])[2] +  '/' + str(volt) +  '/' + str(current)
                self.visa.E36000A_setup_channel(command=command)
            else:
                pass
    ...
    def xlsx_Report(self, **kargs):
        if self.Thermal_die_en.Value == True:
            Data_log = self.visa.Keysight_DataLog_793_101_104(visa='USB0::0x2A8D::0x5101::MY58014090::0::INSTR')
            v1 = Data_log[0]
            v2 = Data_log[1]
            v3 = Data_log[2]
            ...
            result = [self.TestItem] + [self.chip_version] + self.test_condition + self.Test_Info_list + [
                'Hyperlink_Log'] + [''] + self.slice_result + [self.die0_THM] + [self.die1_THM] + [self.die2_THM]
        else:
            result = [self.TestItem] + [self.chip_version] + self.test_condition + self.Test_Info_list + ['Hyperlink_Log'] + self.slice_result
    ...
    def xlsx_write_result(self,Test_Result,Hyperlink_path):
        ...
        for prcs in psutil.process_iter():
            if str(prcs.name).find('EXCEL.EXE') !=-1:  # 印出所有正在執行的應用程式 ( 從中觀察 pid )
        ...
        for i in range(len(Test_Result)):
            ...
            if 'FAIL' in Test_Result or 'abp_failed' in Test_Result:
                font = Font(u'Arial', size=10, bold=True, italic=False, strike=False, color='ff0000')
            else:
                font = Font(u'Arial', size=10, bold=False, italic=False, strike=False, color='000000')
            if Test_Result[i] == 'Hyperlink_Log' :
                font = Font(u'Arial', size=10, bold=False, italic=False, strike=False, color='0000ff')
                Select_sheet.cell(row=Start_row_Num, column=i + 1).hyperlink = Hyperlink_path
                Select_sheet.cell(row=Start_row_Num, column=i + 1).style ='Hyperlink'
                Select_sheet.cell(row=Start_row_Num, column=i + 1).value = 'HYPERLINK LINK'
                Select_sheet.cell(row=Start_row_Num, column=i + 1).alignment = align
                Select_sheet.cell(row=Start_row_Num, column=i + 1).border = border
        ...
        Excel_Path.close()
        if self.eye_graph_en !=0:
            self.xls_graph_result(row=Start_row_Num, col=i, Hyperlink_path=Hyperlink_path)
    ...
    def xlsx_log_file_Name(self, result, sheet_name, test_num):
        ...
        for n in range(len(result)):
            ...
            file_Name = [self.Log_Folder_path] + [self.TestDataRate] + [die_str] + [slice_str] + [vref_str] + [w_s] + [pa] + [self.test_lane] + [self.sw_bist]
            ...
            for D in range(info_len):
                ...
                if die_str.find('-') != -1:
                    Select_sheet.cell(row=Start_row_Num + n, column=col + D).alignment = align
                    Select_sheet.cell(row=Start_row_Num + n, column=col + D).border = border
                    Select_sheet.cell(row=Start_row_Num + n, column=col + D).font = font
                    Select_sheet.cell(row=Start_row_Num + n, column=col + D).value = '--'
                else:
                    if D == 0:
                        Hyperlink_path = 'Test Report Log/' + self.Log_Folder_path + '.txt'
                        Select_sheet.cell(row=Start_row_Num+n, column=col + D).hyperlink = Hyperlink_path
                        Select_sheet.cell(row=Start_row_Num+n, column=col + D).style = 'Hyperlink'
                        Select_sheet.cell(row=Start_row_Num+n, column=col + D).alignment = align
                        Select_sheet.cell(row=Start_row_Num+n, column=col + D).border = border
                        Select_sheet.cell(row=Start_row_Num+n, column=col + D).font = font
                        Select_sheet.cell(row=Start_row_Num+n, column=col + D).value = file_Name[D]
                    else:
                        Select_sheet.cell(row=Start_row_Num+n, column=col + D).alignment = align
                        Select_sheet.cell(row=Start_row_Num+n, column=col + D).border = border
                        Select_sheet.cell(row=Start_row_Num+n, column=col + D).font = font
            ...
            for i in range(len(bin)):
                if bin[i] == '1' or  bin[i] == 'V' or  bin[i] == 'E':
                    ...
                    bin_log=bin[i]
                font = Font(u'Arial', size=10, bold=False, italic=False, strike=False)
                Select_sheet.cell(row=Start_row_Num+n, column=i + info_len+1).alignment = align
                Select_sheet.cell(row=Start_row_Num+n, column=i + info_len+1).border = border
                Select_sheet.cell(row=Start_row_Num+n, column=i + info_len+1).font = font
                Select_sheet.cell(row=Start_row_Num+n, column=i + info_len+1).fill = fill
                Select_sheet.cell(row=Start_row_Num+n, column=i + info_len+1).value = bin_log
    ...
    def xls_graph_result(self, **kargs):
        ...
        Select_sheet = Excel_Path['Test Result']
        font = Font(u'Arial', size=10, bold=False, italic=False, strike=False, color='0000ff')
        HYPERLINK = "=HYPERLINK(\"" + Hyperlink_path[:-3] + 'png' +  "\"," + "\"" + 'Hyperlink_Graph' + "\"" + ")"
    ...
    def xlsx_reg_compare(self, Test_Result):
        ...
        for i in range(len(Test_Result)):
            ...
            for r in range(len(result_list)):
                ...
                Select_sheet.cell(row=Start_row_Num+i, column=r+1).value = result_list[r]
    ...
    def picture_merge(self,folder_path, **kwargs):
        ...
        bg = Image.new('RGB', (pic_width * col, pic_high * row_width), '#000000')  # pic_width x pic_high col / row
    ...
    def eye_scan_even(self, event):
        ...
        if self.eye_scan_en == 0:
            ...
            if scan_event == 'NA':
                for w in range(999999*999999*999999):
                    ...
                    if w == 0:
                        ...
                        self.eye_scan_en == 1
                    self.TestItem_Now_wx.Value = f'Auto Eye Scan - {w+1} (On Going !! )'
                ...
                for s in range(scan_num):
                    if s == 0:
                        ...
                        self.eye_scan_en == 1
                    self.TestItem_Now_wx.Value = f'Auto Eye Scan - {s+1}  (On Going !! )'
            ...
            self.TestItem_Now_wx.Value = '( Create Receiver Eye Scan Graph and Result App )'
    ...
    def eye_scan_window(self):
        ...
        self.scan_info_win = f'Eye Width : Eye Width Left = {self.w_l} , Eye Width Right = {self.w_r}'
        ...
        group1 = tk.LabelFrame(root, text=f'< {self.eye_scan_type.GetValue()} Eye Scan Test Result >',  font=fontStyle2)
        group2 = tk.LabelFrame(root, text='< Color Grade >',  font=fontStyle2)

        center_color_TF = fail_color_1_TF = fail_color_max_TF = 0
        fail_color_r1_TF = fail_color_r2_TF = fail_color_r3_TF = fail_color_r4_TF = fail_color_r5_TF = 0
        fail_color_r6_TF = fail_color_r7_TF = fail_color_r8_TF = fail_color_r9_TF = fail_color_r10_TF = 0
            ...
            for y in range(0, phase_len , 1):
                ...
                if max_value == 1:
                    ...
                    if value == 1:
                        ...
                        color_list = ('#FF7575', '#FF5151', '#FF2D2D', '#FF0000', '#EA0000', '#CE0000', '#930000', '#750000', '#600000' , '#4D0000')
                        if max_value <= 10 :
                            grade = 1
                        else:
                ...
                scan = 15
                if int(self.seach_min.Value) <= value and int(self.seach_max.Value) >= value:
                    fail_count = value
                else:
                    fail_count = ''
                canvas.create_rectangle(scan * y, scan * (x + 1), scan * (y + 1), scan * 2 + (scan * x), fill=color, outline='#fff')
                canvas.create_text(scan * 0.5 + (scan * y), scan * 1.5 + (scan * x), text=fail_count, fill="white", font=('Lucida Bright', 6))
        ...
        tk.Label(group1, text=self.scan_info_win, font=fontStyle2).grid(column=0, row=0, sticky=tk.W)
        tk.Label(group1, text=self.scan_info_high, font=fontStyle2).grid(column=0, row=1, sticky=tk.W)

        tk.Label(group2, text='Center Phase Pass', fg='#006600', font=fontStyle2).grid(column=0, row=0, sticky=tk.W)
        if center_color_TF:
            tk.Label(group2, text='Phase Pass', fg=center_color, font=fontStyle2).grid(column=0, row=1, sticky=tk.W)
        if fail_color_1_TF:
            tk.Label(group2, text='Phase Fail 1 Time', fg=fail_color_1, font=fontStyle2).grid(column=0, row=2, sticky=tk.W)
        if fail_color_max_TF:
            tk.Label(group2, text=f'Phase Fail {max_value} Times', fg=fail_color_max, font=fontStyle2).grid(column=0, row=3, sticky=tk.W)
        if fail_color_r1_TF:
            tk.Label(group2, text=f'Phase Fail 2 To {grade} Times', fg=fail_color_r1, font=fontStyle2).grid(column=0, row=4, sticky=tk.W)
        if fail_color_r2_TF:
            tk.Label(group2, text=f'Phase Fail {grade} To {grade*2} Times', fg=fail_color_r2, font=fontStyle2).grid(column=0, row=5, sticky=tk.W)
        if fail_color_r3_TF:
            tk.Label(group2, text=f'Phase Fail {grade*2} To {grade*3} Times', fg=fail_color_r3, font=fontStyle2).grid(column=0, row=6, sticky=tk.W)
        if fail_color_r4_TF:
            tk.Label(group2, text=f'Phase Fail {grade*3} To {grade*4} Times', fg=fail_color_r4, font=fontStyle2).grid(column=0, row=7, sticky=tk.W)
        if fail_color_r5_TF:
            tk.Label(group2, text=f'Phase Fail {grade*4} To {grade*5} Times', fg=fail_color_r5, font=fontStyle2).grid(column=0, row=8, sticky=tk.W)
        if fail_color_r6_TF:
            tk.Label(group2, text=f'Phase Fail {grade*5} To {grade*6} Times', fg=fail_color_r6, font=fontStyle2).grid(column=0, row=9, sticky=tk.W)
        if fail_color_r7_TF:
            tk.Label(group2, text=f'Phase Fail {grade*6} To {grade*7} Times', fg=fail_color_r7, font=fontStyle2).grid(column=0, row=10, sticky=tk.W)
        if fail_color_r8_TF:
            tk.Label(group2, text=f'Phase Fail {grade*7} To {grade*8} Times', fg=fail_color_r8, font=fontStyle2).grid(column=0, row=11, sticky=tk.W)
        if fail_color_r9_TF:
            tk.Label(group2, text=f'Phase Fail {grade*8} To {grade*9} Times', fg=fail_color_r9, font=fontStyle2).grid(column=0, row=12, sticky=tk.W)
        if fail_color_r10_TF:
            tk.Label(group2, text=f'Phase Fail {grade*9} To {max_value-1} Times', fg=fail_color_r10, font=fontStyle2).grid(column=0, row=13, sticky=tk.W)

        # Buttom setup
        autoButton = tk.Button(root, command=root.destroy,  text=" Close Window ", font=fontStyle2)
        group1.grid()
        group2.grid()
        add=10
    ...
    def print_eye_log(self, result, sheet_name, test_num):
        for n in range(len(result)):
            ...
            w_s = ((result[n].split(':'))[1]).split(',')[0]
            pa = ((result[n].split(':'))[1]).split(',')[1]
            file_Name = [self.Log_Folder_path] + [self.TestDataRate] + [die_str] + [slice_str] + [vref_str] + [w_s] + [pa]
            info_len = len(file_Name)
            if n == 0 or n == 65 :
        ...
        if (self.project_json):
            ...
            if json_ip in self.project_json.keys():
                if len(json_ip.split('_')) >= 2:
                    ...
                    try:
                        instrument_DCAL_get = self.project_json[json_ip]['instrument_DCAL']
    ...
    def event_window(self):
        ...
        button = tk.Button(text="Press Buttom \n( Close Excel )", command=close_window, font=('Lucida Bright', 10, 'bold'), padx=10, pady=10, bg='#f90', bd=5)
