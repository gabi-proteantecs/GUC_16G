...
class UCIe_2p5D:
    ...
    def LR_EN(self, **kargs):
        ...
        if data_training_en != 0:
            for idx in range(len(self.modes)):
                ...
                self.phy.reg_user_set(die_arr=self.die_arr, group_arr=self.group_arr, tx_slice=self.tx_slice, rx_slice=self.rx_slice, reg_arr=LR, mode=init_mode)
    ...
    def FLY_LR_EN(self, **kargs):
        ...
        if data_training_en != 0:
            for idx in range(len(self.modes)):
                ...
                print(f'Init Slice : TX Slice=User Define / RX Slice=User Define (Test Die : Only Force Die0 and Die1)')
                # self.phy.reg_user_set(die_arr=self.die_arr, group_arr=self.group_arr, tx_slice=[0,1,2,3], rx_slice=[0,1,2,3], reg_arr=hw_non_1, mode=init_mode)
                self.phy.reg_user_set(die_arr=self.die_arr, group_arr=self.group_arr, tx_slice=self.tx_slice, rx_slice=self.rx_slice, reg_arr=FLY_LR, mode=init_mode)
    ...
    def CPM(self):
        ...
        for die_number, die_select in zip(die_number_set, die_select_set):
            ...
            self.phy.non_i2c_write(slave, 0x04, 0, 8, PM_en_counter_H)  # PM_en_counter[15:8]
            self.phy.non_i2c_write(slave, 0x05, 0, 8, PM_en_counter_L)  # PM_en_counter[7:0]
        ...
        for DL_select in DL_select_set:
            ...
            for die_number, die_select in zip(die_number_set, die_select_set):
                ...
                self.phy.non_i2c_write(slave, 0xfe, 0, 8, 0x01)  # read PM_Value
                PM_counter_all = []
                for PM_number, PM_address_L, PM_address_H in zip(PM_number_set, PM_address_L_set, PM_address_H_set):
                    PM_counter_L = int(self.phy.non_i2c_read(slave, PM_address_L, 0, 8), 16)  # PM_counter[7:0]
                    PM_counter_H = int(self.phy.non_i2c_read(slave, PM_address_H, 0, 8), 16)  # PM_counter[15:8]
            ...
            write_tb = tabulate(die_PM_all, headers=['Die', 'PM_1', 'PM_2', 'PM_3', 'PM_4', 'PM_5',
                                                     'PM_6', 'PM_7', 'PM_8', 'PM_9'], showindex="always", tablefmt='psql')
            print(write_tb, flush=True)

            Report_time = datetime.datetime.now()
            Report_time = ((str(Report_time)).replace(':', '-'))[0:19]
            Log_Folder_path = 'Test Report\Test Report Log\CPM_' + \
                              chip_num + '_' + str(TestTemp) + 'Degree_' + \
                              str(TestDataRate) + 'Gbps_' + \
                              'DL' + str(DL_select) + '_' + \
                              str(Log_File_Name) + '_' + \
                              str(Report_time) + \
                              '.txt'
    ...
    def Flyover(self, **kargs):
        ...
        TX_FLOV_CLK = self.TX_CLK(self.tx_die, self.tx_group, slice=self.tx_slice, doset=0, r_bk=1) # read
        self.TX_CLK(self.rx_die, self.rx_group, slice=self.rx_slice, doset=0, r_bk=1) # read

        TX_FLOV_D_L = self.TX_D_L(self.tx_die, self.tx_group, slice=self.tx_slice, doset=0, r_bk=1) # read
        self.TX_D_L(self.rx_die, self.rx_group, slice=self.rx_slice, doset=0, r_bk=1) # read

        TX_FLOV_D_H = self.TX_D_H(self.tx_die, self.tx_group, slice=self.tx_slice, doset=0, r_bk=1) # read
        self.TX_D_H(self.rx_die, self.rx_group, slice=self.rx_slice, doset=0, r_bk=1) # read

        TX_FLOV_DRD = self.TX_DRD(self.tx_die, self.tx_group, slice=self.tx_slice, doset=0, r_bk=1) # read
        self.TX_DRD(self.rx_die, self.rx_group, slice=self.rx_slice, doset=0, r_bk=1) # read

        RX_FLOV_CLK = self.RX_CLK(self.tx_die, self.tx_group, slice=self.tx_slice, doset=0, r_bk=1) # read
        self.RX_CLK(self.rx_die, self.rx_group, slice=self.rx_slice, doset=0, r_bk=1) # read

        RX_FLOV_D_L = self.RX_D_L(self.tx_die, self.tx_group, slice=self.tx_slice, doset=0, r_bk=1) # read
        self.RX_D_L(self.rx_die, self.rx_group, slice=self.rx_slice, doset=0, r_bk=1) # read

        RX_FLOV_D_H = self.RX_D_H(self.tx_die, self.tx_group, slice=self.tx_slice, doset=0, r_bk=1) # read
        self.RX_D_H(self.rx_die, self.rx_group, slice=self.rx_slice, doset=0, r_bk=1) # read

        RX_FLOV_DRD = self.RX_DRD(self.tx_die, self.tx_group, slice=self.tx_slice, doset=0, r_bk=1) # read
        self.RX_DRD(self.rx_die, self.rx_group, slice=self.rx_slice, doset=0, r_bk=1) # read

        print(' \n ')

        if TX_FLOV_CLK == RX_FLOV_CLK:
            print(f'TX_FLOV_CLK = {TX_FLOV_CLK}, ' f'RX_FLOV_CLK = {RX_FLOV_CLK}, pass !', flush=True)
        else:
            print(f'TX_FLOV_CLK = {TX_FLOV_CLK}, ' f'RX_FLOV_CLK = {RX_FLOV_CLK}, failed !', flush=True)

        if TX_FLOV_D_L == RX_FLOV_D_L:
            print(f'TX_FLOV_D_L = {TX_FLOV_D_L}, ' f'RX_FLOV_D_L = {RX_FLOV_D_L}, pass !', flush=True)
        else:
            print(f'TX_FLOV_D_L = {TX_FLOV_D_L}, ' f'RX_FLOV_D_L = {RX_FLOV_D_L}, failed !', flush=True)

        if TX_FLOV_D_H == RX_FLOV_D_H:
            print(f'TX_FLOV_D_H = {TX_FLOV_D_H}, ' f'RX_FLOV_D_H = {RX_FLOV_D_H}, pass !', flush=True)
        else:
            print(f'TX_FLOV_D_H = {TX_FLOV_D_H}, ' f'RX_FLOV_D_H = {RX_FLOV_D_H}, failed !', flush=True)

        if TX_FLOV_DRD == RX_FLOV_DRD:
            print(f'TX_FLOV_DRD = {TX_FLOV_DRD}, ' f'RX_FLOV_DRD = {RX_FLOV_DRD}, pass !', flush=True)
        else:
            print(f'TX_FLOV_DRD = {TX_FLOV_DRD}, ' f'RX_FLOV_DRD = {RX_FLOV_DRD}, failed !', flush=True)
    ...
    def Lane_Repair(self, **kargs):
        ...
        LR_D0_DDR = self.LR_D0(self.tx_die, self.tx_group, slice=self.tx_slice, doset=0, r_bk=1) # read
        LR_D1_DDR = self.LR_D1(self.tx_die, self.tx_group, slice=self.tx_slice, doset=0, r_bk=1) # read
        LR_D2_DDR = self.LR_D2(self.tx_die, self.tx_group, slice=self.tx_slice, doset=0, r_bk=1) # read
        LR_D3_DDR = self.LR_D3(self.tx_die, self.tx_group, slice=self.tx_slice, doset=0, r_bk=1) # read
        LR_CLK_DDR = self.LR_CLK(self.tx_die, self.tx_group, slice=self.tx_slice, doset=0, r_bk=1) # read
        LR_VLD_DDR = self.LR_VLD(self.tx_die, self.tx_group, slice=self.tx_slice, doset=0, r_bk=1) # read



        if LR_D0_DDR == '0x00':
            print(f'LR_D0_DDR is {LR_D0_DDR}, ' f' = 0x0, pass !', flush=True)
        else:
            print(f'LR_D0_DDR is {LR_D0_DDR}, ' f' != 0x0, failed !', flush=True)

        if LR_D1_DDR == '0x01':
            print(f'LR_D1_DDR is {LR_D1_DDR}, ' f' = 0x1, pass !', flush=True)
        else:
            print(f'LR_D1_DDR is {LR_D1_DDR}, ' f' != 0x1, failed !', flush=True)

        if LR_D2_DDR == '0x20':
            print(f'LR_D2_DDR is {LR_D2_DDR}, ' f' = 0x20, pass !', flush=True)
        else:
            print(f'LR_D2_DDR is {LR_D2_DDR}, ' f' != 0x20, failed !', flush=True)

        if LR_D3_DDR == '0x21':
            print(f'LR_D3_DDR is {LR_D3_DDR}, ' f' = 0x21, pass !', flush=True)
        else:
            print(f'LR_D3_DDR is {LR_D3_DDR}, ' f' != 0x21, failed !', flush=True)

        if LR_CLK_DDR == '0x0':
            print(f'LR_CLK_DDR is {LR_CLK_DDR}, ' f' = 0x0, pass !', flush=True)
        else:
            print(f'LR_CLK_DDR is {LR_CLK_DDR}, ' f' != 0x0, failed !', flush=True)

        if LR_VLD_DDR == '0x0':
            print(f'LR_VLD_DDR is {LR_VLD_DDR}, ' f' = 0x0, pass !', flush=True)
        else:
            print(f'LR_VLD_DDR is {LR_VLD_DDR}, ' f' != 0x0, failed !', flush=True)
    ...
    def TX_CLK(self, die, group, **kwargs):
        ...
        for slice_n in slice:
            ...
            if doset == 1:
                self.phy.indirect_write(slave, offset + base, bit, int(setv, 16), slice_num=slice_n)
            if r_bk == 1:
                rbv = self.phy.indirect_read(slave, offset + base, bit, slice_num=slice_n)
                rbvs.append(rbv)
        if show == 1:
